...
import { ItemallocationpoitemtaggingComponent } from '../itemallocationpoitemtagging/itemallocationpoitemtagging.component';

@Component({
  selector: 'app-itemallocationpotagging',
  templateUrl: './itemallocationpotagging.component.html',
  styleUrls: ['./itemallocationpotagging.component.scss']
})
export class ItemallocationpotaggingComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    forkJoin(
     ...
     this.DropDownListService.getJwPo(),
  ...
  onChangePo(pur_orderid)
  {
    this.grnlist.forEach(element => {
      if (element.pur_orderid == pur_orderid) {
        this.userForm.patchValue({ grn_date: element.ord_date, grn_no: element.pur_order_no, supplier_name: element.supplier, totalqty: element.tot_wt });
      }
    });
  }
  ...
  detailsPopUp(index) {
   ...
   // let partyitem_qty = this.jw_grn_partytag_details.at(index).get("partyitem_qty").value
    ...
    const dialogRef = this.dialog.open(ItemallocationpoitemtaggingComponent, {
      data: {
      ...
      for (let data1 of jobparty) {
        ...
        this.DropDownListService.getJwGrnPartywitemDetails(jw_grn_tag, data1["party"]).subscribe(itemdynamic => {
          ...
          this.jw_grn_partytag_details.at(k).patchValue({
           ...
           , qty: data1["qty"], allocated_qty: data1["allocated_qty"]
          });
