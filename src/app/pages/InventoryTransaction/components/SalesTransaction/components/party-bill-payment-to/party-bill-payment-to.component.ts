...
@Component({
  selector: 'app-party-bill-payment-to',
  templateUrl: './party-bill-payment-to.component.html',
  styleUrls: ['./party-bill-payment-to.component.scss']
})
export class PartyBillPaymentToComponent implements OnInit {
  @ViewChild(SelectAutocompleteComponent) multiSelect: SelectAutocompleteComponent;
  public userForm:FormGroup;
  model: PartyBillPaymentTo = new PartyBillPaymentTo();
  submitted = false;
  status = false;
  currentDate:any;
  BusinessUnit="0";
  ...
  constructor(public fb:FormBuilder,private Service: Master,private DropDownListService: DropdownServiceService) 
  {
    this.userForm=fb.group({
      ...
      party_bill_payment_to_details: this.fb.array([this.fb.group({
        invoice_no:'',
        invoice_number:'',
        invoice_date:'',
        bill_amt:'',
  ...
  get party_bill_payment_to_details(){return this.userForm.get('party_bill_payment_to_details') as FormArray;}
  ...
  add()
  {
    this.party_bill_payment_to_details.push(this.fb.group({
      invoice_no:'',
      invoice_number:'',
      invoice_date:'',
      bill_amt:'',
  ...
  {
    this.party_bill_payment_to_details.reset();
   ...
   this.userForm.patchValue({total_billamt:null,total_adjamt:null,total_dueamt:null,total_payamt:null}) ;
    ...
    if(Bussunit != '0' && Bussunit != undefined && party_id !='0' && party_id !=undefined)
    {
      this.DropDownListService.getPartyOutstanding(party_id,Bussunit).subscribe(data=>
        {
          ...
          let j = 0;
          this.add();
          while (this.party_bill_payment_to_details.length) 
          this.party_bill_payment_to_details.removeAt(0);
          for(let data1 of data)
           { 
            this.add();
            this.party_bill_payment_to_details.at(j).patchValue({invoice_no:data1.invoice_id,bill_amt:data1.billamount,
      ...
      (x=>
        ...
        let Adj_amt =0;
        let Due_amt =0;        
        for(let i=0;i<this.party_bill_payment_to_details.length;i++)
        {
          Bill_amt+= this.party_bill_payment_to_details.at(i).get("bill_amt").value;
          Adj_amt+= this.party_bill_payment_to_details.at(i).get("adj_amt").value;
          Due_amt+= this.party_bill_payment_to_details.at(i).get("due_amt").value;
  ...
  { 
   let Payable_Amt= this.party_bill_payment_to_details.at(index).get("payable_amt").value
   let Due_amt  = this.party_bill_payment_to_details.at(index).get("due_amt").value;
    
   if(Payable_Amt>Due_amt)
    {
      alert("Payable Amount Must be less or equal to Due Amount: "+Due_amt);
      this.party_bill_payment_to_details.at(index).patchValue({payable_amt:null});
    ...
    {  
      let Payable_amt =0;  
      for(let i=0;i<this.party_bill_payment_to_details.length;i++)
      {Payable_amt+= this.party_bill_payment_to_details.at(i).get("payable_amt").value;}
  ...
  send()
  {
    ...
    for(let i=0;i<this.party_bill_payment_to_details.length;i++)
    {
      this.Payable_Amt= this.party_bill_payment_to_details.at(i).get("payable_amt").value;
    }
    ...
    {    
            this.status = false;
            this.Service.createPartyBillPaymentTo(this.userForm.getRawValue()).subscribe(data => 
            {
              console.log(this.userForm.value);
              alert("Party Bill Payment To created successfully.");
              this.ngOnInit();
